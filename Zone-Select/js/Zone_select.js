(function($){
		...
		init:function(options){
			return this.each(function(){
				...
				if(typeof(settings)== 'undefined'){
					...
					settings=$.extend({},dft, options);
					$this.data('zone',settings)
				}else{
				...
				if(settings.beautify){
					...
					if(settings.select_filter){
						...
						zone_district_beautify='<div><span id="zone_district_val"><a>请选择</a><i></i></span><div><input type="text" ><ul><li></li></ul></div></div>';
					}
					$(".province").append(zone_province_beautify);
					$(".city").append(zone_city_beautify);
					$(".district").append(zone_district_beautify);
				}
			})
		},
	...
	$.fn.zone=function(){
		...
		this.on("click",".zone_select>span>div>span",function(){
			$(".province>div>div,.city>div>div,.district>div>div").css("display","none");
			$(this).next().toggle();
		...
		this.on("click",".zone_select>span>div>div>ul>li",function(){
			...
			if(zone_beautify_txt!=zone_beautify_txt_old){
				...
				if(zone_this_id=='zone_province_val'){
					...
					$(".zone_select .city ul").html(city);
				}else if(zone_this_id=='zone_city_val'){/*市改变时进行的操作*/
					...
					$(".zone_select .district ul").html(city);
				}else  if(zone_this_id=='zone_district_val'){/*县改变时进行的操作*/
					$(".district select").val(zone_beautify_daval);
				}
			}else{
				$(".city>div>div,.district>div>div").css("display","none");
			}
				...
				if(last_keyup-event.timeStamp==0){
					...
					for(let i=0;i<zone_select_li.length;i++){
						if(zone_select_li[i].innerText.indexOf(zone_select_input)==-1){
							...
							zone_select_li[i].style.display="block";
						}
					}
				}
			},300)
